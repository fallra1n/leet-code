...
//type MaxHeap struct {
//	data       []int
//	realToHeap map[int]int // mapping (start pos of element) % k -> pos of element in heap
//	heapToReal map[int]int // mapping pos in heap of element -> (start pos of element) % k
//}
//
//func (mh *MaxHeap) SiftUp(id int) { // id == heapId
//	pId := (id - 1) / 2
//
//	for id != 0 {
//		if mh.data[id] <= mh.data[pId] {
//			break
//		}
//
//		if mh.data[id] > mh.data[pId] {
//			// swapping
//			temp := mh.data[id]
//			mh.data[id] = mh.data[pId]
//			mh.data[pId] = temp
//
//			idRealPos := mh.heapToReal[id]
//			pIdRealPos := mh.heapToReal[pId]
//
//			mh.realToHeap[idRealPos] = pId
//			mh.realToHeap[pIdRealPos] = id
//			mh.heapToReal[id] = pIdRealPos
//			mh.heapToReal[pId] = idRealPos
//
//			id = pId
//			pId = (id - 1) / 2
//		}
...
//		if mh.data[id] < mh.data[max] {
//			temp := mh.data[id]
//			mh.data[id] = mh.data[max]
//			mh.data[max] = temp
//
//			idRealPos := mh.heapToReal[id]
//			sonRealPos := mh.heapToReal[max]
//
//			mh.realToHeap[idRealPos] = max
//			mh.realToHeap[sonRealPos] = id
//			mh.heapToReal[id] = sonRealPos
//			mh.heapToReal[max] = idRealPos
//		}
//
//		id = max
//	}
//}
//
//func (mh *MaxHeap) BuildMaxHeap(data []int) {
//	mh.data = data
//	mh.heapToReal = make(map[int]int)
//	mh.realToHeap = make(map[int]int)
//	size := len(mh.data)
//
//	for i := 0; i < size; i++ {
//		mh.heapToReal[i] = i
